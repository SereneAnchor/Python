...
class MaxHeap:
	...
	def shiftUp(self,index):
		while index>0:
			...
			if self.items[index]>self.items[parent]:
				...
				index=parent
			#当前节点<=父节点,不需要交换,仍然保持最大堆结构
			else:
				break

	#向下调整:当前节点和左右子节点比较,找出较大节点并进行交换;继续往下找
	def shiftDown(self,index):
		while True:
			...
			if large==index:
				break
			#交换当前节点和三者中较大的节点
			self.items[index],self.items[large]=self.items[large],self.items[index]
			#index下移指向较大的节点
			index=large
	...
	def popItem(self):
		...
		#保存堆顶元素
		top=self.items[0]
		#移除数组中最后一个元素(树中右下角的元素)
		last=self.items.pop()
		#移除后堆为空说明堆中只有一个元素,不能向下继续执行;否则堆内至少还有一个元素
		if not self.isEmpty():
			#用最后一个元素替代堆顶
			self.items[0]=last
			#从堆顶向下调整堆
